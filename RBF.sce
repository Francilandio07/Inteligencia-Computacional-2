...
// p -> Número de atributos (34)
// N -> Número de amostras (358)
[p N] = size(P);
...
//Normalizando os Centroides
for i = 1:p
    C(i, :) = (C(i, :) - mean(C(i, :)))/stdev(C(i, :));
end

// Largura da função radial (sigma)
G = 25;

// Quantidade Q de neurônios ocultos por N colunas (35x180)
N = size(P_treino)(2);
Z_treino = zeros(Q, N);
...
for i = 1:N
    for j = 1:Q
        // ===Função de Ativação===
        // (Entradas - Centroide)
        v = norm(P_treino(:, i) - C(:, j));
        
        // Função gaussiana
        // cada input passa pelos Q neurônios
        // Q linhas (valores/neuronios) para cada amostra        
        Z_treino(j, i) = exp(-(1/[2*G^2])*v^2);
    end
end

// matriz de output da camada oculta com o bias
Z_treino = [ones(1, N); Z_treino];

// Mínimos quadrados -> pesos da camada de saída
// [6 x (Q+1)] classes x (neuronios + bias)
W = T_treino * Z_treino' * (Z_treino * Z_treino')^(-1);

Nt = size(P_teste)(2); //Número de dados para teste
Z_teste = zeros(Q, Nt);

//Preparando os vetores do teste
for i = 1:Nt
    for j = 1:Q
        v = norm(P_teste(:, i) - C(:, j));      
        Z_teste(j, i) = exp(-(1/[2*G^2])*v^2); 
    end
end

// Matriz de output da camada oculta com o bias
Z_teste = [ones(1, Nt); Z_teste];

// Matriz com as previsões dos dados de testes
previsao = W*Z_teste;

count = 0;

//Percorrendo as previsões e respectivos valores corretos
for i = 1:Nt
    [a b] = max(previsao(:, i)); 
    [c d] = max(T_teste(:, i));
    //Comparar índice do valor previsto com o índice correto
    if(b == d) then
        count = count + 1; //Registro dos acertos
    end
end

//Resultado
percent = 100*(count/Nt); //Porcentagem de acertos nos dados de teste.
printf('\n\n O percentual de acertos dessa rede RBF é %3.2f%%.', percent);
